...
const InterestedVolunteeringOpportunities = ({ opportunities }) => {  // Define component with opportunities prop
  const [sortOption, setSortOption] = useState("all");  // Initialize state for sort option and its setter

  const handleSortOptionChange = (e) => {   // Define event handler for sort option change
    setSortOption(e.target.value);          // Update sort option state with new value
  };

  const sortOpportunities = (opportunities) => {  // Define function to sort opportunities based on sort option state
    if (sortOption === "matchedInterests") {      // If sort option is matchedInterests
      // Sort by matched interests (descending order)
      return opportunities.sort((a, b) => b.matched_interests - a.matched_interests);
    } else if (sortOption === "mostRecent") {     // If sort option is mostRecent
      // Sort by most recent (descending order)
      return opportunities.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
    } else {                                      // If sort option is all or any other value
  ...
  return (
    <div>
      ...
      {sortedOpportunities.length > 0 ? (     // If there are sorted opportunities
        sortedOpportunities.map((opportunity) => (   // Map over the sorted opportunities
...
InterestedVolunteeringOpportunities.propTypes = {   // Define propTypes for the component
